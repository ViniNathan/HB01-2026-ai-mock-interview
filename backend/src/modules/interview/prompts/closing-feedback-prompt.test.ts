...
describe("buildClosingFeedbackPrompt", () => {
  ...
  it("includes thread-based evaluation guardrails", () => {
    const prompt = buildClosingFeedbackPrompt({
      level: "mid",
      resumeSummary: sampleSummary,
    });

    expect(prompt).not.toContain("Candidate utterances");
    expect(prompt).not.toMatch(/\n1\. /);
    expect(prompt).toContain("messages thread");
    expect(prompt).toContain("role `human`");
    expect(prompt).toContain("role `ai`");
    expect(prompt).toContain("non-substantive");
    expect(prompt).toContain("Background context only");
    expect(prompt).toContain("final feedback for the entire interview");
  });
});
